		...
		let promise = 
			authPromise
				.then(auth => {
					...
					console.log(`Start call getRowsFromStartRow`);
				...
				.then(resRowsFromStartRow => {
					console.log(`Success call getRowsFromStartRow`);
					let rows = resRowsFromStartRow.values;
					if (!rows || rows.length == 0) {
						console.log('Fail to getRowsFromStartRow');
						return new Promise((resolve, reject) => reject('Fail to getRowsFromStartRow'));
					}
