...
let buildReport = function(){
	...
	let fs = require('fs');
	let lunchMoneyConfig = JSON.parse(fs.readFileSync(`${__dirname}/lunchMoneyConfig.json`).toString());
	...
	function sheetRange(){
		...
		let fs = require('fs');
		let _ = require(`${__dirname}/lib/util`);
		let lunchMoneyConfigStat = fs.statSync(`${_.getPath('cache')}/lunchMoneyConfig.json`);
		let mtime = lunchMoneyConfigStat.mtime;
