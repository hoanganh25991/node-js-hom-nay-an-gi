...
let parseUserText = function(req){
	...
	let userTextInfo = {
		...
		cmd:  text_chunks[0],
		user_name,
		sheet_name: state.users[user_name],
		response_url,
		command
	};

	let log_file = _.getPath('cache/history.log');
	let fs       = require('fs');
	let moment   = require('moment');
	let os       = require('os');
	let record   = `[${moment().utcOffset(7 * 60).format('YYYY-MM-DD HH:mm:ss')}] ${JSON.stringify(userTextInfo)}${os.EOL}`;
	...
	switch(userTextInfo['cmd']){
		...
		case 'menu':
			...
			userTextInfo['whichDate'] = whichDate(date);
			break;
		...
		case 'order':
			if(text_chunks[1]){
				...
				if(!isNaN(dishIndex)){
					userTextInfo['dishIndex'] = dishIndex;
					userTextInfo['whichDate'] = whichDate(undefined);
					// return;
				}else{
					dishIndex = parseInt(text_chunks[2], 10);
					if(!isNaN(dishIndex)){
						userTextInfo['dishIndex'] = dishIndex;
						userTextInfo['whichDate'] = whichDate(text_chunks[1]);
						// return;
					}
				}
			}
		...
		case 'name':
			...
			userTextInfo['new_name'] = user_name_arr.join(' ');
			break;
	}
