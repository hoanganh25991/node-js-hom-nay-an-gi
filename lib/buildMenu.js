let buildMenu = function (res){
	 ...
	 */
	/**
	 * Consider FIRST array is the RULE
	 */
	chunkedRows = chunkedRows.map(chunkedRow => {
		let baseChunkedRowLength = chunkedRow[0].length;

		chunkedRow = chunkedRow.map( col => {
			return col.filter((x, i) => {
				return i < baseChunkedRowLength;
			});
		});
